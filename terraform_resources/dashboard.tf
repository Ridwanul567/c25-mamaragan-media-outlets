...
resource "aws_ecr_repository" "dashboard_repo" {
  name                 = "${var.resource_prefix}-dashboard-repo"
  image_tag_mutability = "MUTABLE"
}

# CloudWatch Log Group
resource "aws_cloudwatch_log_group" "dashboard_logs" {
  name              = "/ecs/${var.resource_prefix}-dashboard"
  retention_in_days = 7
}
...
data "aws_iam_policy_document" "dashboard_execution_policy_doc" {
  statement {
    ...
    resources = ["*"] # API constraint: GetAuthorizationToken requires "*"
  }
...
resource "aws_iam_role" "dashboard_execution_role" {
  name               = "${var.resource_prefix}-dashboard-execution-role"
...
resource "aws_iam_policy" "dashboard_execution_policy" {
  name   = "${var.resource_prefix}-dashboard-execution-policy"
...
resource "aws_iam_role" "dashboard_task_role" {
  name               = "${var.resource_prefix}-dashboard-task-role"
  assume_role_policy = data.aws_iam_policy_document.ecs_trust_policy_doc.json
}

resource "aws_iam_policy" "dashboard_task_policy" {
  name   = "${var.resource_prefix}-dashboard-task-policy"
...
resource "aws_ecs_task_definition" "dashboard_task" {
  family                   = "${var.resource_prefix}-dashboard-task"
  network_mode             = "awsvpc"
...
# Output ECR Repository URL for your GitHub Actions Workflow
output "dashboard_ecr_repository_url" {
  value       = aws_ecr_repository.dashboard_repo.repository_url
  description = "The ECR Repository URL for pushing dashboard container images"
}